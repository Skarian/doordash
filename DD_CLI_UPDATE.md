...
6. Use `port/launcher.py` to select
   `keyrings.alt.file.PlaintextKeyring` on this headless exe.dev VM.
...
- `port/src/`, `port/build.sh`, and `port/launcher.py` own the upstream-derived
  ...
  intentionally.
- `ops/doordash-auth/` owns local exe.dev browser automation, the interactive
  login flow, the unattended renewal flow, noVNC assets, and systemd templates.
...
- `/home/exedev/.local/share/python_keyring/keyring_pass.cfg` — the CLI's saved
  OAuth access-token record. This headless port uses a plaintext keyring, so
  the file must remain owned by `exedev` with mode `0600`. Never copy it into
  the workspace, vendor archives, build artifacts, logs, or rollback bundles.
- `/home/exedev/.local/share/dd-cli/chrome-profile` — the durable Chrome profile
  containing the DoorDash browser identity session used for automatic OAuth
  approval. Preserve the directory in place with mode `0700`; do not move it
  back under `/tmp`, wipe it during build cleanup, or include it in artifacts.

After installing a new binary:

1. Do not run a broad cleanup against `/home/exedev/.local/share`,
   `/home/exedev/.config`, or `/home/exedev/.local`; replace only the intended
   CLI binary and build outputs.
2. Do not change `ops/doordash-auth/` unless the update intentionally changes
   local authentication operations. If its systemd templates changed,
   reinstall them under `/etc/systemd/system/`, run `systemctl daemon-reload`,
   and re-enable the timer.
3. Run `systemctl start dd-cli-token-refresh.service` and require a successful
   exit. This confirms that the preserved browser session can renew OAuth
   without user interaction.
4. Confirm `dd-cli-token-refresh.timer` is enabled and active with a next run
...
5. Confirm the test left no Chrome/Xvfb processes and no listeners on ports
   ...
   `bash ops/doordash-auth/mobile-login.sh` once; do not delete the credential
...
- [x] `DD_CLI_CA_BUNDLE` works with the system CA bundle in the exe.dev HTTPS
  environment.
